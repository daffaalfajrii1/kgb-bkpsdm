...
@if (session('login_blokir_skp'))
    <div role="alert" class="mb-4 rounded-xl border-l-4 border-amber-500 bg-amber-50 text-amber-950 px-4 py-4 text-sm shadow-sm">
        ...
        <p class="leading-relaxed text-amber-900/95">
            {{ session('pesan_blokir_login_skp', $errors->first('nip')) ?: 'Anda tidak dapat masuk karena hasil penilaian kinerja (SKP) pada 1 (satu) tahun terakhir berada pada kategori yang memblokir akses, sesuai data penilaian kinerja yang tercatat.' }}
        </p>
    </div>
@elseif ($errors->any())
